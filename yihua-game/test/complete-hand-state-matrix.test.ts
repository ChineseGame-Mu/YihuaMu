...
describe("complete hand and table-state matrix", () => {
  it.each([
    ...
    [
      [joker("small"), joker("small"), joker("big"), joker("big")],
      "joker-bomb",
    ],
  ...
  it("covers wheel straights, approved 23456, and rejects high-wrap 2 straights", () => {
    expect(
      classifyHand([
        card("A", "clubs"),
        card("2", "hearts"),
    ...
    ).toMatchObject({ kind: "straight", highRank: "6" });
    expect(
      classifyHand([
        card("J", "clubs"),
        card("Q", "hearts"),
  ...
  it("orders bomb families across normal, straight-flush, six-card, and joker bombs", () => {
    const four = classifyHand([
      card("8"),
      card("8", "hearts"),
      card("8", "spades"),
      card("8", "diamonds"),
    ]);
    const five = classifyHand([
      card("7"),
      card("7", "hearts"),
      card("7", "spades"),
      card("7", "diamonds"),
  ...
  it("keeps first-round opening draw as an explicit table-machine phase", () => {
    ...
    if (opening.phase !== "opening-draw")
      throw new Error("expected opening-draw phase");
  ...
  it("rejects table-machine actions that are illegal for the current phase", () => {
    const lobby = createLobbyState(4, 0);
    expect(() =>
      transitionGame(lobby, { type: "next-round" }, () => 0.5),
