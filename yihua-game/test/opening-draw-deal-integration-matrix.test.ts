...
import { dealAfterOpeningDraw, type OpeningDrawState } from "../src/core/game-state.js";
  ...
  for (const playerCount of supportedCounts) {
    it(`carries the unique opening winner into the ${playerCount}-player first deal`, () => {
      const machine = completeOpeningDrawMachine(
        createOpeningDrawMachine(createDeck(playerCount), playerCount, deterministicRandom),
      );
