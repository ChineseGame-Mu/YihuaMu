...
const lowPair: readonly Card[] = [suited("5", "clubs"), suited("5", "diamonds")];
const highPair: readonly Card[] = [suited("6", "clubs"), suited("6", "diamonds")];
  ...
  it.each(SUPPORTED_PLAYER_COUNTS)(
    ...
    (playerCount) => {
      let state = completeTableOpeningDraw(
        createTableRoundState(openingDeck(playerCount), playerCount, KEEP_ORDER, LEVEL_RANK),
      );

      expect(state.openingDraw.winnerSeat).toBe(0);
      expect(state.trick?.currentTurn).toBe(0);
      expect(classifyHandWithLevel(lowPair, LEVEL_RANK)).toMatchObject({ kind: "pair" });
      expect(classifyHandWithLevel(highPair, LEVEL_RANK)).toMatchObject({ kind: "pair" });
      ...
      const passingSeats = Array.from({ length: playerCount - 1 }, (_, index) =>
        (index + 2) % playerCount,
      ).filter((seat) => seat !== 1);
