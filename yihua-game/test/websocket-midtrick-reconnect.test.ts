...
describe("websocket mid-trick reconnect recovery", () => {
  for (const playerCount of SUPPORTED_PLAYER_COUNTS) {
    it(`restores public trick state after play and pass for ${playerCount} players`, async () => {
      ...
      if (afterPlay.phase !== "playing") throw new Error("single play completed round");
      ...
      expect(gameState?.handCounts).toEqual(afterPass.hands.map((hand) => hand.length));
      expect(gameState?.leadingPlay).toEqual({ seat: leader, cards: [leadCard] });
