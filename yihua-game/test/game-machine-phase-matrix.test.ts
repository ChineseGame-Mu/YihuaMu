...
import { transitionGame, type GameMachineAction } from "../src/core/game-machine.js";
import { completeRound, createLobbyState, type GameState } from "../src/core/game-state.js";
...
const makeStates = (): Record<GameState["phase"], GameState> => {
  ...
  const playing = transitionGame(opening, { type: "deal-after-opening-draw" }, fixedRandom);
  if (playing.phase !== "playing") {
    throw new Error("playing phase expected");
  }
  const completed = completeRound(playing, playing.currentTurn);
  return { lobby, "opening-draw": opening, playing, "round-complete": completed };
};
...
describe("game machine phase/action rejection matrix", () => {
  it("rejects every action that is illegal for each phase before domain validation", () => {
    ...
    const allowed: Record<GameState["phase"], ReadonlySet<GameMachineAction["type"]>> = {
    ...
    for (const [phase, state] of Object.entries(states) as [GameState["phase"], GameState][]) {
