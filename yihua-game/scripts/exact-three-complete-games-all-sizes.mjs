...
const playCompleteGame = (initialState) => {
  ...
  while (state.phase === "playing") {
    if (actions >= actionLimit)
      throw new Error(`game exceeded ${actionLimit} actions`);
    const seat = state.currentTurn;
    const hand = state.hands[seat];
    if (!hand || hand.length === 0)
      throw new Error(`invalid current hand at seat ${seat}`);
  ...
  if (state.finishedSeats.length !== state.config.playerCount)
    throw new Error("finish order incomplete");
  if (new Set(state.finishedSeats).size !== state.config.playerCount)
    throw new Error("finish order duplicated");
  if (state.winnerSeat !== state.finishedSeats[0])
