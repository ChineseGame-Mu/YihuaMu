...
if (!existsSync(prettierCli)) {
  fail(
    "local Prettier is not installed. Run `yarn install --frozen-lockfile` in frontend first.",
  );
}
...
const supported = new Set(["version", "check", "write", "staged"]);
if (!supported.has(mode)) {
  fail(`unsupported mode '${mode}'. Use version, check, write, or staged.`);
}
...
if (mode === "version") {
  console.log(`[prettier-341-guard] PASS: Prettier ${REQUIRED_PRETTIER}`);
  process.exit(0);
}
...
if (mode === "check") {
  ...
  console.log(
    `[prettier-341-guard] PASS: frontend uses Prettier ${REQUIRED_PRETTIER} and src is formatted.`,
  );
  process.exit(0);
}
...
if (mode === "write") {
  ...
  runPrettier(["src", "--check"]);
  console.log(
    `[prettier-341-guard] FIXED: frontend src formatted with Prettier ${REQUIRED_PRETTIER}.`,
  );
  process.exit(0);
}
...
const stagedFrontendFiles = stagedOutput
  ...
  .map((file) => ({
    repoPath: file,
    frontendPath: relative(frontendRoot, resolve(repoRoot, file)),
  }));

if (stagedFrontendFiles.length === 0) {
  console.log(
    `[prettier-341-guard] PASS: no staged frontend files need formatting; version ${REQUIRED_PRETTIER} verified.`,
  );
  process.exit(0);
}
...
execFileSync(
  "git",
  ["add", "--", ...stagedFrontendFiles.map(({ repoPath }) => repoPath)],
  {
    cwd: repoRoot,
    stdio: "inherit",
  },
