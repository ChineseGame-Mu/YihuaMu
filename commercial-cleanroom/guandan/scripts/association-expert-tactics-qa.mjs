...
const suited = (id, rank, suit = "clubs") => ({
  id,
  copy: 0,
...
export const runAssociationExpertTacticsQa = () => {
  ...
  const structuredHand = [
    suited("s3c", "3"),
    suited("s3d", "3", "diamonds"),
    suited("s4c", "4"),
    suited("s4d", "4", "diamonds"),
    suited("s7c", "7"),
    suited("s8c", "8"),
    suited("s9c", "9"),
    suited("s10c", "10"),
  ...
  result.roleSelectionFromLegalInformation = has(
    normalLead,
    "roleSelectionFromLegalInformation",
  );
  result.minimumSufficientOvertake = has(
    normalLead,
    "minimumSufficientOvertake",
  );
  result.playedCardMemory = has(normalLead, "playedCardMemory");
  result.remainingCardInferenceWithoutHiddenInfo = has(
    normalLead,
    "remainingCardInferenceWithoutHiddenInfo",
  );
  result.bombConservation = has(normalLead, "bombConservation");
  result.leadSmallBurdenWithoutBreakingStructure =
    normalLead.type === "play" &&
    normalLead.reason === "structured-low-burden-lead";

  const partnerLeadCards = [suited("pl7", "7")];
  const yieldDecision = chooseExpertAction({
    ...base,
    hand: structuredHand,
    leadingPlay: lead(2, partnerLeadCards),
    seat: 0,
    handCounts: [structuredHand.length, 12, 10, 12],
  });
  result.partnerYielding =
    yieldDecision.type === "pass" && has(yieldDecision, "partnerYielding");
  ...
  const catchDecision = chooseExpertAction({
    ...
    leadingPlay: null,
    seat: 0,
    handCounts: [feedHand.length, 9, 0, 9],
    finishedSeats: [2],
  });
  result.partnerCatchLeadExploitation = has(
    catchDecision,
  ...
  result.upgradeOutcomeOptimization = has(
    endDecision,
    "upgradeOutcomeOptimization",
  );
  result.endgameModeSwitch =
    result.endgameModeSwitch && has(endDecision, "endgameModeSwitch");
  ...
  const wildcardDecision = chooseExpertAction({
    ...base,
    hand: wildcardHand,
    leadingPlay: lead(1, leadingPair),
    seat: 0,
    handCounts: [wildcardHand.length, 9, 9, 9],
  });
  result.wildcardValueOptimization =
    wildcardDecision.type === "play" &&
    has(wildcardDecision, "wildcardValueOptimization");

  const bombHand = [
    suited("b9c1", "9"),
    suited("b9d1", "9", "diamonds"),
    suited("b9h1", "9", "hearts"),
    suited("b9s1", "9", "spades"),
    suited("b9c2", "9"),
    suited("b9d2", "9", "diamonds"),
  ];
  const straightFlush = [
    suited("sf4", "4", "spades"),
    suited("sf5", "5", "spades"),
  ...
  const bombDecision = chooseExpertAction({
    ...base,
    hand: bombHand,
    leadingPlay: lead(1, straightFlush),
    seat: 0,
    handCounts: [bombHand.length, 2, 10, 10],
  });
  result.bombForControlWithFollowup =
    bombDecision.type === "play" &&
    has(bombDecision, "bombForControlWithFollowup");
