...
const provider = readFileSync(new URL("../../frontend/src/GuandanWebsocketProvider.tsx", import.meta.url), "utf8");
describe("clean-room frontend reconnect isolation", () => {
  ...
  it("clears queued old-socket messages before reconnect snapshots", () => {
    ...
    expect(provider).toContain('clearQueuedMessages(); setStatus("connecting")');
    expect(provider).toContain('websocketRef.current = null; clearQueuedMessages(); setStatus("disconnected")');
  });
  it("uses bounded exponential reconnect backoff", () => {
    expect(provider).toContain("Math.min(1000 * 2 ** reconnectAttemptRef.current, 10000)");
