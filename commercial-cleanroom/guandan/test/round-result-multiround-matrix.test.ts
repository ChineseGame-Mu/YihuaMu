...
describe("automatic round result three-round matrix", () => {
  it.each(PLAYER_COUNTS)(
    "preserves winner-led lifecycle for three complete %i-player rounds",
    (playerCount) => {
      const opening = { attempts: [], winnerSeat: 0 } as const;
      let playing = createState(playerCount);

      for (let round = 1; round <= 3; round += 1) {
        const completed = finishRound(playing);
        expect(completed.phase).toBe("round-complete");
        if (completed.phase !== "round-complete") {
          throw new Error(`round ${round} did not complete`);
        }
        ...
        if (next.phase !== "playing") {
          throw new Error(`round ${round + 1} did not start`);
        }
