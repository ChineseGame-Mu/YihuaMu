...
use axum::{extract::ws::{Message, WebSocketUpgrade}, response::{IntoResponse, Redirect}, routing::{get, post}, Extension, Json, Router};
...
use axum::{body::{Empty, Full}, extract::Path, response::Response};
...
lazy_static::lazy_static! {
    static ref CARDS_JSON: CardsBlob = CardsBlob { cards: FULL_DECK.iter().map(|c| c.as_info()).collect() };
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    #[cfg(not(feature = "dynamic"))]
    let app = app.route("/", get(|| async { serve_static_routes(Path("index.html".to_string())).await })).route("/*path", get(serve_static_routes));
    let cors = {
        ...
        if allowed_origins.trim() == "*" {
            ...
            if origins.is_empty() { info!(ROOT_LOGGER, "No valid CORS origins configured, using same-origin policy"); CorsLayer::new() }
    ...
    info!(ROOT_LOGGER, "Shutting down"); Ok(())
}

#[derive(Debug, Serialize, Deserialize)]
struct GameStats { num_games_created: u64, num_active_games: usize, num_players_online_now: usize, sha: &'static str }
...
async fn handle_websocket(ws: WebSocketUpgrade, Extension(backend_storage): Extension<HashMapStorage<VersionedGame>>, Extension(stats): Extension<Arc<Mutex<InMemoryStats>>>) -> impl IntoResponse {
    ws.on_upgrade(|ws| {
        ...
        tokio::task::spawn(async move { while let Some(v) = rx.recv().await { let _ = user_ws_tx.send(Message::Binary(v)).await; } debug!(logger_, "Ending tx task"); });
        let logger_ = logger.clone();
