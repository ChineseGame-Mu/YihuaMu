...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let cors = {
        ...
        if allowed_origins.trim() == "*" {
            ...
            if origins.is_empty() { info!(ROOT_LOGGER, "No valid CORS origins configured, using same-origin policy"); CorsLayer::new() } else { info!(ROOT_LOGGER, "CORS origins configured: {:?}", origins); CorsLayer::new().allow_origin(AllowOrigin::list(origins)).allow_methods([Method::GET, Method::POST, Method::OPTIONS]).allow_headers(tower_http::cors::Any) }
        }
    };
...
async fn handle_websocket(ws: WebSocketUpgrade, Extension(backend_storage): Extension<HashMapStorage<VersionedGame>>, Extension(stats): Extension<Arc<Mutex<InMemoryStats>>>) -> impl IntoResponse {
    ws.on_upgrade(|ws| {
        ...
        tokio::task::spawn(async move {
            ...
            loop {
                tokio::select! {
                    maybe_v = rx.recv() => { match maybe_v { Some(v) => { if user_ws_tx.send(Message::Binary(v)).await.is_err() { break; } } None => break, } }
                    _ = heartbeat.tick() => { if user_ws_tx.send(Message::Ping(Vec::new())).await.is_err() { break; } }
                }
            }
